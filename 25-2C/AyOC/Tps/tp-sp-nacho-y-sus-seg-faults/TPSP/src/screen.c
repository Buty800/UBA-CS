...
void screen_draw_layout(void) {
  uint8_t bodyAttr   = C_BG_BLUE | C_FG_WHITE;
  uint8_t headerAttr = C_BG_BLUE | C_FG_LIGHT_BROWN;
  uint8_t boxAttr    = C_BG_BLUE | C_FG_BLACK;

  screen_draw_box(0, 0, ROWS, COLS, ' ', bodyAttr);

  screen_draw_box(0, 0, 3, COLS, ' ', boxAttr);
  print("System Programming - Modo Protegido", 2, 1, headerAttr);

  screen_draw_box(3, 0, ROWS-3, 2, ' ', boxAttr);
  screen_draw_box(3, COLS-2, ROWS-3, 2, ' ', boxAttr);

  print("Integrantes: Ignacio Romero - Bautista De Suto - Silvano Picard", 4, 5, 0x0F);
  print("Grupo: Nacho y sus seg faluts", 4, 7, 0x0F);
}
