...
static const uint32_t identity_mapping_end = 0x3FFFFF;
...
paddr_t mmu_next_free_kernel_page(void) {
  paddr_t p = next_free_kernel_page;
...
paddr_t mmu_init_task_dir(paddr_t phy_start) {
  paddr_t task_pd_pa = mmu_next_free_kernel_page();
  paddr_t stack_pa = mmu_next_free_user_page();
  zero_page(task_pd_pa);
  

  mmu_map_page(task_pd_pa, (vaddr_t)TASK_STACK_BASE - 0x1000, stack_pa, MMU_P | MMU_U | MMU_W );
  mmu_map_page(task_pd_pa, (vaddr_t)TASK_SHARED_PAGE, SHARED, MMU_P|MMU_U|MMU_W);
  mmu_map_page(task_pd_pa, (vaddr_t)TASK_CODE_VIRTUAL, phy_start, MMU_P|MMU_U);
  mmu_map_page(task_pd_pa, (vaddr_t)TASK_CODE_VIRTUAL + 0x1000, phy_start + 0x1000, MMU_P|MMU_U);

  
  for(uint32_t kernel_page_dir = 0x0;kernel_page_dir <= identity_mapping_end; kernel_page_dir += 0x1000){
    mmu_map_page(task_pd_pa, kernel_page_dir, kernel_page_dir, MMU_W|MMU_P);
...
bool page_fault_handler(vaddr_t virt) {
  ...
  if (virt >= ON_DEMAND_MEM_START_VIRTUAL && virt <= ON_DEMAND_MEM_END_VIRTUAL) {
    uint32_t cr3 = rcr3();
    mmu_map_page(cr3,
                 ON_DEMAND_MEM_START_VIRTUAL,
                 ON_DEMAND_MEM_START_PHYSICAL,
                 MMU_P| MMU_U|MMU_W);
    return true;
  }
