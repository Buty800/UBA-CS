...
bool hay_accion_que_toque(accion_t* accion, char* nombre) {
	while(accion){
		if(!strcmp(accion->destino->nombre, nombre)) return true;
...
void invocar_acciones(accion_t* accion, tablero_t* tablero) {
	while(accion){
		...
		if(accion->destino->en_juego){
			...
			if(!accion->destino->vida) accion->destino->en_juego = 0;
		}
	...
	*cant_rojas = *cant_azules = 0;
	...
	for(int i = 0; i < ALTO_CAMPO; i++){
		for(int j = 0; j < ANCHO_CAMPO; j++){
			...
			if(carta){
				if(carta->jugador == JUGADOR_AZUL) (*cant_azules)++;
				if(carta->jugador == JUGADOR_ROJO) (*cant_rojas)++;
			}
		}
	}
