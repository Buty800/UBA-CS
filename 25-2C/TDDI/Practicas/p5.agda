...
-- split xs 0 prf = ([] , (xs , (refl , refl)))
-- split (x ∷ xs) (suc k) prf = {!   !}
...
∈-decidible {n} {[]} = no (λ ())  
∈-decidible {n} {x ∷ xs} with x ≟ n 
... | yes refl = yes zero
... | no  x≠n with ∈-decidible {n} {xs}
... | yes x∈tail = yes (suc x∈tail) 
... | no  x∉tail = no help  
  where
    help : ¬ (n ∈ (x ∷ xs)) 
    help zero = x≠n refl
...
List-igualdad-decidible dec-eq-A [] (_ ∷ _) = no (λ ()) 
List-igualdad-decidible dec-eq-A (_ ∷ _) [] = no (λ ())
